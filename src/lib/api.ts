...
const generateZKPHash = async (content?: string) => {
  if (typeof crypto !== "undefined" && crypto.subtle && crypto.subtle.digest) {
    ...
    return `integrity-sha256:0x${hex}`;
  }
  return `integrity-sha256:0x${Date.now().toString(16)}`;
};
...
export const api = {
  ...
  async getTokensBySchool(schoolId: string): Promise<SchoolToken[]> {
    const response = await fetchWithTimeout(
      `${API_URL}/tokens?schoolId=${schoolId}`,
    );
    if (!response.ok) return [];
    const data = await response.json();
    return data.map((t: any) => ({
  ...
  async updateIntervention(id: string, data: Partial<ProtectionIntervention>) {
    const payload: any = {};
    if ((data as any).ticketId !== undefined)
      payload.ticket_id = (data as any).ticketId;
    if ((data as any).victimAlias !== undefined)
      payload.victim_alias = (data as any).victimAlias;
    if ((data as any).schoolOrigin !== undefined)
      payload.school_origin = (data as any).schoolOrigin;
    if (data.category !== undefined) payload.category = data.category;
    if (data.urgency !== undefined) payload.urgency = data.urgency;
    if (data.stage !== undefined) payload.stage = data.stage;
    if ((data as any).shelterRequired !== undefined)
      payload.shelter_required = (data as any).shelterRequired;
    if ((data as any).assignedPsychologist !== undefined)
      payload.assigned_psychologist = (data as any).assignedPsychologist;
    if ((data as any).assignedLegalAid !== undefined)
      payload.assigned_legal_aid = (data as any).assignedLegalAid;
  ...
  async factoryReset() {
    const response = await fetchWithTimeout(`${API_URL}/factory-reset`, {
      method: "POST",
    });
