...
const INITIAL_DATA = {
  ...
  users: [
    {
      ...
      password_hash: 'JTI0MmElMTAkcmU3YVl2WkV3VlphcWRSNmFhRldIeS5Nai8ueHVsVzh3LlEuWjYuNHZ2Lnp5dnZ2dnZ2dg==', 
      role: 'guru',
      ...
      permissions: ['Triage Laporan', 'Chat Siswa', 'Catatan Rahasia', 'Eskalasi Kasus']
    },
    {
      id: 'usr-admin-01',
      ...
      permissions: ['Manajemen Token', 'Kelola Petugas BK', 'Audit Log', 'Konfigurasi Sistem']
    },
    {
      id: "usr-disdik-01",
      ...
      permissions: ["Pengawasan Wilayah", "Monitoring Respon Sekolah", "Indeks Kerawanan", "Pemberian Supervisi"]
    },
    {
      ...
      permissions: ["Intervensi Kritis", "Disposisi Psikolog", "Layanan Rumah Aman", "Pendampingan Hukum"]
    }
  ],
  audit_logs: [],
  interventions: [],
  news_articles: [
    {
      id: "news-1",
      title: "Sosialisasi Permendikbudristek No. 46 Tahun 2023: Sekolah Wajib Bentuk Satgas PPKSP",
      ...
      excerpt: "Setiap satuan pendidikan kini diwajibkan membentuk Tim Pencegahan dan Penanganan Kekerasan (TPPK).",
      content: [
  ...
  help_articles: [
    {
      ...
      excerpt: "Panduan 4 langkah mudah melapor tanpa khawatir identitas bocor atau diketahui teman sekelas.",
      content: [
  ...
  faq_items: [
    {
      id: "faq-1",
      question: "Apakah Guru BK atau Wali Kelas bisa mengetahui siapa yang mengirim laporan?",
      answer: "Tidak. Sistem TAMENG tidak menyimpan identitas pelapor, email, nomor ponsel, nama perangkat, maupun alamat IP. Laporan hanya berisi nomor acak (Tiket). Guru BK hanya menerima informasi mengenai kejadian yang Anda ceritakan tanpa mengetahui siapa Anda.",
    ...
    messages: [
      {
        id: crypto.randomUUID(),
        sender_type: 'system',
        message_text: 'Laporan Anda berhasil dienkripsi dan diterima oleh Tim Bimbingan Konseling (BK) & Satgas PPKSP Sekolah.',
        created_at: new Date().toISOString(),
        is_encrypted: true
      }
    ]
...
app.get('/api/tickets/:recoveryCode', (req, res) => {
  const db = getDB();
  const ticket = db.tickets.find(t => t.recovery_code === req.params.recoveryCode);
...
app.get('/api/tokens', (req, res) => {
  ...
  const tokens = schoolId ? db.tokens.filter(t => t.school_id === schoolId) : db.tokens;
  res.json(tokens);
});
...
app.put('/api/tokens/:id/status', (req, res) => {
  const db = getDB();
  const index = db.tokens.findIndex(t => t.id === req.params.id || t.token_code === req.params.id);
  if (index === -1) return res.status(404).json({ error: 'Token not found' });
...
app.post('/api/login', (req, res) => {
  ...
  if (!user) {
    return res.status(401).json({ error: 'User tidak ditemukan atau role tidak sesuai' });
  }
...
app.put('/api/interventions/:id', (req, res) => {
  ...
  if (index === -1) return res.status(404).json({ error: 'Intervention not found' });
...
app.get('/api/dashboard/stats', (req, res) => {
  ...
  tickets.forEach(t => {
    if (t.messages && t.messages.length > 0) {
      const firstCounselorMsg = t.messages.find(m => m.sender_type === 'counselor');
  ...
  res.json({
    ...
    pendingTickets: tickets.filter(t => t.status === 'diterima' || t.status === 'ditinjau').length,
    resolvedTickets: resolvedTickets.length,
    avgResponseTime: respondedCount > 0 ? Math.round((totalResponseHours / respondedCount) * 10) / 10 : 0
  });
});
...
app.post('/api/account-requests', (req, res) => {
  ...
  const existing = db.account_requests.find(r => r.email === req.body.email && r.status === 'pending');
  if (existing) return res.status(400).json({ error: 'Pengajuan dengan email ini sudah dalam antrean.' });
  const newRequest = {
    id: crypto.randomUUID(),
    name: req.body.name,
    email: req.body.email,
